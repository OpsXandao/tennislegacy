...
class TorneioATP250:
    ...
    def jogar_qualy(self, jogadores_qualy):
        ...
        nomes_confrontos = [a["nome"] for a, _ in confrontos] + [b["nome"] for _, b in confrontos]
        if self.jogador_nome not in nomes_confrontos:
            jogador_dict = {"nome": self.jogador_nome, "nacionalidade": self.jogador_nacionalidade}
    ...
    def obter_proximo_adversario(self, nome_jogador):
        ...
        for a, b in estado["rodadas"].get(fase, []):
            if nome_normalizado in [normalizar_nome(a), normalizar_nome(b)]:
                ...
                return adversario if isinstance(adversario, dict) else {"nome": adversario}
        return None
    ...
    def _atualizar_fase_se_necessario(self, estado):
        ...
        for r in estado["resultados"][fase]:
            if isinstance(r, dict):
                ...
                if id_vencedor is not None:
                    ...
                    jogador_obj = self.ranking.ranking[id_vencedor - 1] if id_vencedor - 1 < len(self.ranking.ranking) else r["vencedor"]
                else:
                    # fallback caso id_ranking falhe
                    jogador_obj = self.ranking.buscar_jogador_por_nome(r["vencedor"]["nome"])
        ...
        if fase == "qualy_1":
            if len(vencedores) != 8:
                raise ValueError("❌ Esperado exatamente 8 vencedores para montar a 'qualy_2'.")
        ...
        # 🚩 qualy_2: 4 vencedores para pre_oitavas + 12 do ranking
            ...
            if len(vencedores) != 4:
                raise ValueError("❌ Esperado exatamente 4 vencedores para montar a 'pre_oitavas'.")
            jogadores_chave = self.ranking.ranking[:12] + vencedores  # 12 + 4 = 16
            random.shuffle(jogadores_chave)
            estado["rodadas"]["pre_oitavas"] = list(zip(jogadores_chave[::2], jogadores_chave[1::2]))
    ...
    def _simular_partida_npc(self, a, b):
        ...
        return vencedor, perdedor, f"{nome_vencedor} {sets_v} x {sets_d} {nome_perdedor}"
    ...
    def simular_npcs_na_fase_atual(self, nome_jogador):
        ...
        for a, b in estado["rodadas"].get(fase, []):
            ...
            if nome_normalizado not in nomes_confronto:
                ...
                vencedor, perdedor, resultado = self._simular_partida_npc(jogador_a, jogador_b)
        ...
        if ultima_fase:
            ...
            for r in estado["resultados"].get(ultima_fase, []):
                if isinstance(r, dict):  # novo formato
                    nome_vencedor = r["vencedor"]["nome"] if isinstance(r["vencedor"], dict) else str(r["vencedor"])
        ...
        for r in estado["resultados"][fase]:
            ...
            if isinstance(r, dict) and isinstance(r.get("vencedor"), dict):
                ...
                if id_rank and 1 <= id_rank <= len(self.ranking.ranking):
                    jogador_obj = self.ranking.ranking[id_rank - 1]  # Ranking começa em 1
            if not jogador_obj:
                # fallback para nome limpo
                if isinstance(r, dict):
                    vencedor_nome = r["vencedor"]["nome"] if isinstance(r["vencedor"], dict) else str(r["vencedor"])
                else:
                    vencedor_nome = extrair_nome_puro(r)
                jogador_obj = self.ranking.buscar_jogador_por_nome(vencedor_nome)
            if jogador_obj:
    ...
    def processar_resultado_partida(self, jogador, adversario, vencedor, resultado_str):
        ...
        confrontos_restantes = [
            (a, b) for a, b in confrontos
            if set([normalizar_nome(a.get("nome", a)), normalizar_nome(b.get("nome", b))]) != set([jogador_norm, adversario_norm])
        ]
        ...
        resultado_dict = {
            ...
            "resultado": resultado_str.split(":")[1].strip() if ":" in resultado_str else resultado_str
        }

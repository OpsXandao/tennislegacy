...
function FormaTab() {
  ...
  const [partidas, setPartidas] = useState<{
    adversario: string; resultado: string | null; torneio: string | null
    semana: number | null; ano: number | null; venceu: boolean
  }[]>([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    api.jogador.formaRecente()
      .then((r) => { setForma(r.forma); setPartidas(r.partidas) })
      .catch(() => {})
      .finally(() => setLoading(false))
  }, [])

  if (loading) {
    return <div className="text-center py-12 arcade-font text-[10px] text-[#444]">CARREGANDO...</div>
  }
    ...
    <div className="space-y-4">
      ...
      {partidas.length > 0 && (
        <div className="border border-[#333] bg-[#111]">
          ...
          {partidas.map((p, i) => (
            <div key={i} className={`flex items-center justify-between px-3 py-3 border-b border-[#1a1a1a] border-l-4 ${p.venceu ? 'border-l-[#00ff88]' : 'border-l-[#ff0055]'}`}>
              <div className="min-w-0">
                <div className="arcade-font text-[10px] text-white truncate max-w-[200px]">{p.adversario}</div>
                <div className="arcade-font text-[8px] text-[#555] mt-0.5">{p.torneio ?? ''} {p.semana ? `S${p.semana}` : ''}</div>
              </div>
              <div

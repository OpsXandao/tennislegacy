...
export function CarreiraTab({ carreira, meusTitulos }: Props) {
  ...
  return (
    <div className="space-y-3">
      <div className="border-2 border-neon-yellow bg-[#111] p-4">
        <div className="grid grid-cols-3 gap-3 text-center mb-4">
          <div>
            <div className="arcade-font text-[8px] text-[#888]">FASE</div>
            <div className="arcade-font text-[9px] text-neon-yellow mt-1">{carreira.fase?.toUpperCase()}</div>
          </div>
          <div>
          ...
          <div>
            ...
            <div className="pixel-font text-xl text-neon-green">{carreira.pico_carreira}a</div>
          </div>
        </div>
        <div>
          ...
          <div className="relative h-3 bg-[#222] border border-[#2a2a2a]">
            <div className="absolute top-0 bottom-0 w-0.5 bg-neon-green" style={{ left: `${picoPct}%` }} />
            <div className="absolute top-0 bottom-0 w-1 bg-neon-yellow" style={{ left: `${idadePct}%` }} />
          </div>
          <div className="flex justify-between arcade-font text-[7px] mt-1">
            <span className="text-neon-yellow">▲ você ({carreira.idade}a)</span>
            <span className="text-neon-green">| pico ({carreira.pico_carreira}a)</span>
          </div>
        </div>
      </div>

      <div className="border border-neon-green/20 bg-[#111] p-3">
        ...
        <div className="h-2 bg-[#222]">
          <div className="h-full bg-neon-green" style={{ width: `${xpPct}%` }} />
        </div>
      </div>

      <div className="border-2 border-neon-yellow/30 bg-[#111] p-3">
        <div className="arcade-font text-[9px] text-neon-yellow mb-3">TÍTULOS ({sumario.Total})</div>
        <div className="grid grid-cols-3 gap-2 text-center mb-3">
          {[
            { label: 'TOTAL', count: sumario.Total, color: 'var(--neon-yellow)' },
            { label: 'SLAMS', count: sumario.GS, color: 'var(--neon-green)' },
            { label: 'MASTERS', count: sumario.M1000, color: 'var(--neon-cyan)' },
          ].map((s) => (
        ...
        {meusTitulos.length > 0 && (
          <button
            ...
            className="mt-3 w-full arcade-font text-[9px] text-neon-yellow border border-neon-yellow/20 py-2 hover:bg-neon-yellow/5"
          >

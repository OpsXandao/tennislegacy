    ...
    <AnimatePresence>
      ...
      >
        <div className="flex items-center justify-between p-4 border-b border-white/10">
          <div>
            <div className="arcade-font text-[11px] text-neon-yellow">{torneio.toUpperCase()}</div>
        ...
        {loading ? (
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {data.campeao_simples && (
              <div className="border-2 border-neon-yellow bg-neon-yellow/10 p-3 flex items-center gap-3">
                <Trophy size={18} className="text-neon-yellow shrink-0" />
                <div>
                  <div className="pixel-font text-[8px] text-[#888]">CAMPEÃO SIMPLES</div>
                  <div className="arcade-font text-[11px] text-neon-yellow">
            ...
            {data.campeao_duplas && (
              <div className="border-2 border-neon-cyan bg-neon-cyan/10 p-3 flex items-center gap-3">
                <Users size={18} className="text-neon-cyan shrink-0" />
                <div>
                  <div className="pixel-font text-[8px] text-[#888]">CAMPEÃO DUPLAS</div>
                  <div className="arcade-font text-[11px] text-neon-cyan">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="arcade-font text-[9px] text-neon-green">
              ...
              return (
                <div className="space-y-4">
                  {fases.map((fase) => {
                    ...
                    return (
                      <div key={fase}>
                        <div className="pixel-font text-[8px] text-neon-yellow mb-2">
                          {fase.toUpperCase()}
                        </div>
                        <div className="space-y-1">
                          {res.map((r, i) => (
                            ...
                            >
                              <span className={r.vencedor === r.j1 ? 'text-neon-green' : 'text-white/50'}>
                                {r.j1}
                              </span>
                              <span className="text-[#888] mx-1">vs</span>
                              <span className={r.vencedor === r.j2 ? 'text-neon-green' : 'text-white/50'}>
                                {r.j2}
                              </span>
                              {r.placar && (
                                <span className="text-neon-yellow ml-2">{r.placar}</span>
                              )}
                            </div>
                          ))}
                          {conf

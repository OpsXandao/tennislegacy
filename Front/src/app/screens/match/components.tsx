...
export function ScoutingReport({
  ...
  return (
    <div className="space-y-4 border p-4" style={{ borderColor: `${accent}40`, background: `${accent}0a` }}>
      ...
      <div className="grid grid-cols-3 gap-2">
        {stats.map(s => (
          <div key={s.label} className="border bg-black/40 p-1.5 text-center" style={{ borderColor: `${accent}20` }}>
            <div className="arcade-font mb-1 text-[9px] text-[#888]">{s.label.toUpperCase()}</div>
      ...
      <div className="space-y-3">
        <div className="flex flex-wrap gap-1.5">
          {report.pontosFortes.map(tag => (
            <span key={tag} className="arcade-font border border-[#00ff88]/20 bg-[#00ff88]/10 px-1.5 py-0.5 text-[7px] text-[#00ff88]">
              {tag}
            </span>
          ))}
          {report.fraquezas.map(tag => (
            <span key={tag} className="arcade-font border border-[#ff4466]/20 bg-[#ff4466]/10 px-1.5 py-0.5 text-[7px] text-[#ff4466]">
              {tag}
            </span>
          ))}
        </div>
        ...
        <div className="space-y-2">
          ...
          {report.dicas.length > 0 && (
            <div className="space-y-1 border-l-2 border-[#ffe600] bg-[#ffe600]/5 p-2">
              <div className="arcade-font text-[8px] tracking-widest text-[#ffe600]">DICA TÁTICA</div>
              {report.dicas.map((dica, idx) => (
                <div key={idx} className="arcade-font text-[9px] leading-snug text-[#d6f2ff]">
                  • {dica}
                </div>
              ))}
            </div>
...
export function OpponentScouting({ adv, accent, superficie }: { adv: AdversarioInfo; accent: string; superficie: string }) {
